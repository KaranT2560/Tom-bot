...
#ki3 = LINETCR.LINE()
#ki3.login(token="EmG8d1abUYpSLEHNK6j7.9a6NQcpsaxK3V2fwENu75W.5vQD2ZPeRrAn/8JBrUSM2hcY4CdKgEy18v7BJZulrOQ=")
#ki3.login(qr=True,)
#ki3.loginResult()

#ki4 = LINETCR.LINE()
#ki4.login(token="Em0d1VSrS4d4YlM3Vcw4.DUg13EeqAEKmm8C/APuSfa.IVBXgOBbj02ijbTAz9j23S4nGmCuui2Yl9JgviRQvT4=")
#ki4.login(qr=True)
#ki4.loginResult()
...
helpMessage ="""==💑➴ω.ざ➻ನণधຮัथໂਹ๊धのัथയ💑==

•[Mybot] คิกเก้อทั้งหมด
•[Me] ฉัน
•[Mid]เช๊คเอมไอดี
•[Gid]
•[All mid] เช็คเอมไอดีทั้งหมด
•[TL:「Text]
•[Mybio:「Text」]เปลี่ยนชื่อคิกเก้อ
•[Cn:「Text」]
•[Mid:「mid」]
•[Contact 「On/Off」]เปิด/ปิด คอนแทรก
•[Auto Join「On/Off」]เปิด/ปิด เข้ากลุ่มอัตโนมัติ
•[Add 「On/Off」]เปิด/ปิด รับแอด
•[Share 「On/Off」]เปิด/ปิดแชร์
•[Jam 「On/Off」]เปิด/ปิด ชื่อนาฬิกา
•[Leave 「On/Off」]เปิด/ปิดเข้าร่วมแชตรวม
•[Group Cancel:]ลบกลุ่มรัน
•[Jam Say:「Nama」]
•[Update]อัพเดทเวลา
•[Banlist]เช็คคนติดดำ
•[Pesan Cek]
•[Blocklist]บร๊อคดำ
•[Pesan set:「Text」]
•[Groups]จำนวลกลุ่มที่มี
•[Get contact:「mid」]

====[Command In Groups]====
●[Kick:「mid」]สั่งเตะจากmid
●[Invite:「mid」]เปิดเชิญจากmid
●[Cancel]ลบค้างเชิญ
●[Ourl]เปิดลิ้ง
●[Mbl]
●[Curl]ปิดลิ้ง
●[Protect 「On/Off」]การป้องกันห้อง
●[Qr 「On/Off」]เปิด/ปิด ลิ้ง
●[Cancel 「On/Off」]เปิด/ปิด ยกเลิกค้างเชิญ
●[Invite 「On/Off」]เปิด/ปิด การเชิญ
●[Ginfo]เช๊คสถานะห้อง
●[Gn 「Nama Grup」]เปลี่ยนลื่อห้อง
●[Album:「ID」]
●[Gurl 「ID」]
●[Nk「nama」] สั่งเตะ
●[Ban] สั่งแบน
●[Unban] ลบแบน
☺[บอทเข้า] เรียกคิกเข้า
☺[บอทออก] สั่งคิกออก
☺[แอบ]ตั้งค่าดูคนอ่าน
☺[คนอ่าน]ดูคนอ่าน
☺[wc]  ดูสถานะห้อง
☺[แท๊ก]  แท๊กชื่อทั้งงห้อง
♨[mybot]  ดูคิกเก้อทั้งหมด
♨[ลบค้างเช้ญ] ลบที่เชิญค้างออก
...
Bots=[mid,kimid,"ufdbb6fd09838be1bbb8a7a8253e4b479"]
admsa = "ufdbb6fd09838be1bbb8a7a8253e4b479"
...
wait = {
    ...
    'timeline':False,
    'autoAdd':False,
    'message':"""ขอบคุณที่เพื่มเพี่นค่ะ Nong Koy""",
    "lang":"JP",
...
def bot(op):
    try:
        ...
        if op.type == 25:
            ...
            if msg.toType == 0:
                msg.to = msg.from_
                if msg.from_ == "ufdbb6fd09838be1bbb8a7a8253e4b479":
                    if "join:" in msg.text:
...
#-----------------------------------------------------------
            ...
            elif msg.text.lower() == 'แอบ':
                    cl.sendText(msg.to, "ตั้งค่าดูคนแอบ")
                    try:
...
#-----------------------------------------------
                ...
                cl.sendText(msg.to,"💞💏ผู้สร้างห้อง💏💞 " + str(ginfo.name) + " :\n" + ginfo.creator.displayName )
            ...
            elif msg.text.lower() == 'koy ping':
                ki.sendText(msg.to,"koy ping 􀠁􀆩􏿿")
                ki2.sendText(msg.to,"koy Ping 􀠁􀆩􏿿")
                ki3.sendText(msg.to,"koy Ping 􀠁􀆩􏿿")
                ki4.sendText(msg.to,"koy พร้อมบินจ้า 􀠁􀆩􏿿")
...
#-----------------------------------------------
        ...
        if op.type == 15:
            ...
            cb = Message()
            cb.to = op.param1
            cb.text = cl.getContact(op.param2).displayName + " ออกจากกลุ่ม " + group.name + " แล้ว"
            cl.sendMessage(cb)
...
# ----------------- NOTIFED MEMBER JOIN GROUP
        ...
        if op.type == 17:
            if op.param2 in Bots:
                return
            cl.sendText(op.param1,cl.getContact(op.param2).displayName + "\n💑💖สวัสดีจ้า ยินดีต้อนรับน๊ะ💖💏\n😂😍มาใหม่แนะนำตัวด้วยน๊ะ😍😂\n")
            print ("MEMBER HAS JOIN THE GROUP")
#-------------------NOTIFED MEMBER OUT GROUP
        if op.type == 19:
            if op.param2 in Bots:
                return
            cl.sendText(op.param1,cl.getContact(op.param2).displayName + " \n😨😈ซัดเต็มข้อเลยครับ ท่านผู้ชม😈😨 ")
